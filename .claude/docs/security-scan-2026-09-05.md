...
- **Estado: FIXED (2026-09-05).** Se agregó `Usuario.es_admin` (migración `0004_usuario_es_admin.py`, promueve a admin a todos los usuarios preexistentes para no perder acceso) y `deps.get_current_admin`. `POST /api/usuarios` y `DELETE /api/usuarios/{id}` ahora exigen admin; `PUT /api/usuarios/{id}` permite que cualquier usuario edite su propio nombre/contraseña, pero exige admin para tocar `activo`/`es_admin` o para editar a otra persona, y bloquea quitarle `es_admin` al único admin restante. `seed_usuario.py` crea el primer usuario como admin. Frontend (`UsuariosAdmin.tsx`, `AuthContext`) actualizado para ocultar "Nuevo usuario" y las acciones sobre otros usuarios a quien no es admin, y muestra/edita el rol. Pendiente: correr `alembic upgrade head` contra la base real antes de desplegar. `admin.py` (planes/consultas), `productos.py` y `clientes.py` siguen sin distinción de rol — evaluar si corresponde extenderlo, no se tocó en este fix para no ampliar el alcance.
